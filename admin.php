...
if ($isAdmin === true) {
    if (isset($_POST["uj-program-gomb"  ])) {
        ...
        if (count($ujprogramhibak) === 0) {
            ...
            $programok = loadData($programfile);
            $programok["programok"][] = $program;
            saveData($programfile, $programok);
        } else {
    ...
    if (isset($_POST["program-torlese-gomb"])) {
        ...
        if ($torlessiker === true) {
            saveData($programfile, ["programok" => $ujprogramok]);
        }
    }

    // allat-fajta, allat-kep, allat-leiras
    $allathibak = [];
    $allatfile = "fiokok/allatok.json";
    if (isset($_POST["allat-hozzaadasa"])) {
        if (!isset($_POST["allat-fajta"]) || trim($_POST["allat-fajta"]) === "") {
            $allathibak[] = "Kötelező megadni állat fajta!";
        }
        if (!isset($_POST["allat-leiras"]) || trim($_POST["allat-leiras"]) === "") {
            $allathibak[] = "Kötelező megadni állat leírást!";
        }
        if (!isset($_FILES["allat-kep"]) || $_FILES["allat-kep"]["error"] !== 0) {
            $allathibak[] = "Kötelező feltőlteni egy képet";
        }

        $allatfajta = $_POST["allat-fajta"];
        $allatleiras = $_POST["allat-leiras"];
        $allatkepfajl = $_FILES["allat-kep"];
        $elfogadottfajlkiterjesztesek = ["png", "jpg", "jpeg"];

        $fajlkiterjesztes = explode(".", $allatkepfajl["name"]);
        $fajlkiterjesztes = strtolower(end($fajlkiterjesztes));

        $allatkeputvonal = "allatkepek/" . $allatfajta;

        if ($allatkepfajl["size"] > 2097152) {
            $allathibak[] = "Túl nagy a fájl mérete!";
            echo 'tul nagy meret';
        }
        ...
        if (count($allathibak) === 0 && $megfelelokiterjesztes) {
            if (move_uploaded_file($_FILES["allat-kep"]["tmp_name"], $allatkeputvonal . "." . $fajlkiterjesztes)) {
                $allat = [
                    "allat-fajta" => $allatfajta,
                    "allat-leiras" => $allatleiras,
                    "allat-kep" => $allatfajta . "." . $fajlkiterjesztes,
                    "allat-kor" => rand(1, 20)
                ];

                $allattomb = loadData($allatfile);
                $allattomb["allatok"][] = $allat;
                saveData($allatfile, $allattomb);
            } else {
                $allathibak[] = "Nem sikerült feltölteni a fájlt!";
            }
        }
    }
} else {
...
<main>
    ...
    <form id="uj-allat-hozzaadasa" method="POST" autocomplete="off" enctype="multipart/form-data">
        <fieldset>
            ...
            <input type="submit" name="allat-hozzaadasa" value="Hozzáadás">
            <?php
            if (isset($_POST["allat-hozzaadasa"])) {
                if (isset($allathibak) && count($allathibak) === 0) {
                    ...
                    if (isset($allathibak) && count($allathibak) >= 1) {
                        foreach ($allathibak as $allathiba) {
                            echo "<p style='text-align: center; font-size: 12px'>" . $allathiba . "</p>";
                        }
                    }
                }
            }
